...
public class PredictionForSentimentCommand implements ActionCommand<ApplicationConfiguration, PredictionForSentiment, Inputs, Outputs> {
    ...
    @Inject
    public PredictionForSentimentCommand(Provider<AuthenticatedWho> authenticatedWhoProvider) {
    ...
    public ActionResponse<Outputs> execute(ApplicationConfiguration configuration, ServiceRequest serviceRequest, Inputs inputs) {
        ...
        var post = new HttpPost("https://api.einstein.ai/v2/language/sentiment");
        ...
        try (var response = HttpClients.createDefault().execute(post)) {
            ...
            throw new RuntimeException("Unable to load the sentiment", e);
        }
